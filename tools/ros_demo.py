...
def parse_config():
    ...
    parser.add_argument('--cfg_file', type=str, default='cfgs/da-nuscenes-coda_models/dsvt_pillar_coda.yaml',
                        help='specify the config for demo')
    parser.add_argument('--pc', '--point_cloud_topic', type=str, default='/ouster/points',
                        help='specify the point cloud ros topic name')
    parser.add_argument('--ckpt', type=str, default='../ckpts/checkpoint_epoch_100.pth', help='specify the pretrained model')
...
def main():
    ...
    dummy_pc = np.random.rand(1000, 4).astype(np.float32)
    # dummy_pc = np.random.rand(1000, 5).astype(np.float32)
