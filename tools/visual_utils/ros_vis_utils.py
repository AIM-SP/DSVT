...
def pcnp_to_datadict(pc_np, dataloader, frame_id=0):
    expected_feature_dim = len(dataloader.point_feature_encoder.src_feature_list)
    if pc_np.shape[1] < expected_feature_dim:
        pad_dim = expected_feature_dim - pc_np.shape[1]
        pc_np = np.hstack([pc_np, np.zeros((pc_np.shape[0], pad_dim), dtype=pc_np.dtype)])
...
def visualize_3d(model, dataloader, pc_msg, bbox_3d_pub, color_map, logger=None):
    ...
    # pc_data = pc2.read_points(pc_msg, field_names=("x", "y", "z", "intensity", "timestamp"), skip_nans=True)
    ...
    pred_cls_scores = pred_dicts[0]['pred_scores'].detach().cpu().numpy()
